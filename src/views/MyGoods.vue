<template>
  <div id="merchantGoods" class="page">
    <div class="weui-panel weui-panel_access">
      <div class="weui-panel__hd">
        <a class="weui-form-preview__btn" @click="addGoods()">新增商品</a>
      </div>
      <div class="weui-panel__hd">我的商品列表</div>

      <div class="weui-panel__bd">
        <div class="weui-media-box weui-media-box_appmsg" v-for="good in myGoods" :key="good.id">
          <div class="weui-media-box__hd">
            <img
              class="weui-media-box__thumb"
              :src="good.headImgUrl | patchImgHttp"
              v-if="good.headImgUrl"
            >
            <img
              class="weui-media-box__thumb"
              src="data:image/png;base64,"
              alt
              v-else
            >
          </div>

          <div class="weui-media-box__bd">
            <div class="right__item_line">{{good.name}}</div>
            <div class="right__item_line">
              <label class="line__label">价格：</label>
              <span class="price">￥{{good.sellingPrice}}</span>
            </div>
            <div class="right__item_line">
              <label class="line__label">标价：</label>
              <span class="price">￥{{good.pricing}}</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { getGoodsPage } from "@/api/merchant";
export default {
  name: "merchantGoods",
  data() {
    return {
      myGoods: []
    };
  },
  mounted() {
    this.loadPage();
  },
  methods: {
    async loadPage() {
      const data = await getGoodsPage({ pageNo: this.pageNo }).catch(e => {
        console.log(e);
      });
      console.log("data", data);
      if (data) {
        this.myGoods = data.dataResult;
      }
    },
    addGoods() {
      this.$router.push("/goodsForm");
    }
  }
};
</script>

<style>
</style>
